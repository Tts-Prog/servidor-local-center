...
export const PropostaController = {
    ...
    async getAll(req, res) {
        const getAllPropostasResponse = await PropostaModel.getAll();
    ...
    async get(req, res) {
        ...
        const getPropostaByIdResponse = await PropostaModel.get(id);
    ...
    async update(req, res) {
        const { id } = req.params;
        const updatedProposta = req.body;
        if (!id) {
            const response = {
                status: "error",
                message: "ID obrigatorio",
        ...
        const updatePropostaResponse = await PropostaModel.update(id, updatedProposta);
    ...
    async aceitar(req, res) {
        ...
        try {
            ...
            const propostaAceitadoResponse = await PropostaModel.aceitarProposta(id);
    ...
    async delete(req, res) {
        const { id } = req.params;
        try {
            const propostaResponse = await PropostaModel.delete(id);
            if (!propostaResponse)
                return res.status(400).json({ message: "Erro ao deletar proposta" });
    ...
    // trabalho final..................................................
    async getByPrestacaoServico(req, res) {
        const { id } = req.params;
        try {
            const propostaResponse = await PropostaModel.get(id);
            if (!propostaResponse)
                return res.status(400).json({ message: "Erro ao buscar proposta" });
