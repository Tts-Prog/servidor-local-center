...
export const OrcamentoController = {
    ...
    async get(req, res) {
        ...
        const getOrcamentoByIdResponse = await OrcamentoModel.get(id);
    ...
    async update(req, res) {
        const { id } = req.params;
        const updatedOrcamento = req.body;
        if (!id) {
            const response = {
                status: "error",
                message: "ID obrigatório",
    ...
    async calculateBudget(req, res) {
        ...
        const prestacaoServico = await PrestacaoServicoModel.getByIdOrcamento(id);
        if (!prestacaoServico) {
            const response = {
                status: "error",
        ...
        const acceptedProposal = proposals.find((proposal) => proposal.estado === EstadoProposta.ACEITE);
        if (!acceptedProposal) {
            const response = {
                status: "error",
        ...
        const updatedOrcamentoResponse = await OrcamentoModel.updateBudget(id, subtotal);
