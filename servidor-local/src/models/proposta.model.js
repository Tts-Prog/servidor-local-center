...
export const PropostaModel = {
    async create(proposta) {
        try {
            const result = await db.query(`INSERT INTO tbl_proposta 
    ...
    async update(id, proposta) {
        try {
            const result = await db.query(`UPDATE tbl_proposta
    ...
    async aceitarProposta(id) {
        try {
            ...
            await db.query(`UPDATE tbl_proposta SET estado = 'Aceite' WHERE id = $1`, [id]);
            // 2. buscar proposta
            const result = await db.query(`SELECT * FROM tbl_proposta WHERE id = $1`, [id]);
            const proposta = result.rows[0];
            // 3. atualizar prestacao_servico
            await db.query(`UPDATE tbl_prestacao_servico 
             SET estado = 'Aceite'
             WHERE id = $1`, [id]);
            // 4. rejeitar restantes propostas
            await db.query(`UPDATE tbl_proposta 
             SET estado = 'Rejeitada'
             WHERE id_prestacao_servico = $1 AND id != $2`, [id]);
            return null;
        }
        catch (error) {
            console.log(error);
    ...
    async delete(id) {
        try {
            const result = await db.query(`DELETE FROM tbl_proposta 
                WHERE id = $1`, [id]);
            return result.rows[0];
        }
        catch (err) {
    ...
    async getByPrestacaoServico(idPrestacaoServico) {
        try {
            const result = await db.query(`SELECT * FROM tbl_proposta 
                    WHERE tbl_proposta.id_prestacao_servico = $1`, [idPrestacaoServico]);
